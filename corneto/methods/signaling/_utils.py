"""Shared internal utilities for signaling methods."""
...
def get_incidence_matrices_of_edges(G, as_dataframe=False, sparse=False):
    ...
    Parameters:
        G: The graph object.
        as_dataframe: Whether to return matrices as pandas DataFrames.
        sparse: Whether to use sparse incidence matrices.
...
def get_egdes_with_head(G):
    ...
    Returns:
        edges_with_head (array): Indices of edges with a head node.
    """
    _, Ah = get_incidence_matrices_of_edges(G)
...
def get_inhibited_nodes(G, exp_list):
    """Return inhibited nodes for every experiment.

    Each column in the result indicates whether a node is inhibited in the
